...
class CacheableResponseSubscriber implements EventSubscriberInterface {
  ...
  public function onRespond(ResponseEvent $event) {
    ...
    if ($response instanceof CacheableResponseInterface) {
      ...
      // Get the max allowed byte length for the Surrogate-key header
      // Can be overriden per site via settings.php
      $limit = $this->configFactory->get('pantheon_advanced_page_cache.settings')->get('surrogate_key_header_limit') ?? 25000;
      ...
      if (strlen($tags_string) > $limit) {
        ...
        $this->logger->log(RfcLogLevel::WARNING, 'More cache tags were present than could be passed in the Surrogate-Key HTTP Header due to length constraints. To avoid a 502 error the list of surrogate keys was trimmed. You can increase the default limit of 25000 bytes by overriding "surrogate_key_header_limit" in settings.php. Since keys beyond the limit were removed this page will not be cleared from the cache when any of the removed keys are cleared (usually by entity save operations) as they have been stripped from the surrogate key header. See https://www.drupal.org/project/pantheon_advanced_page_cache/issues/2973861 for more information about how you can filter out redundant or unnecessary cache metadata.');
      }
